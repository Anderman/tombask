...
struct __attribute__((packed)) Frame25_5
{
    ...
    uint8_t a23;         // 7d 00 00 00 00 aa 06 01 00 00 00 00 00 6a 6d 00 97 14 19
    ...
    uint16_t PowerTotal; // 00 aa 06 01 00 00 00 00 00 6a 6d 00 97 14 19
    ...
    uint16_t PowerComp;  // 00 97 14 19
    uint8_t a40;         // 14 19
    uint8_t a41;         // 19
};
...
static Frame25_5 previousFrame;

void read_25_5(const uint8_t *buffer)
{
    memcpy(&previousFrame, buffer, sizeof(Frame25_5));
    ...
    {
        // Swap bytes voor endianness
        uint16_t powerTotal = (previousFrame.PowerTotal >> 8) | (previousFrame.PowerTotal << 8);
        uint16_t powerComp = (previousFrame.PowerComp >> 8) | (previousFrame.PowerComp << 8);
        int power = powerComp == 0 ? (powerTotal == 0 ? 7 : powerTotal - 28) : powerTotal - 30;
        controlValues.Power = power > 0 ? power : 0;
    }
}